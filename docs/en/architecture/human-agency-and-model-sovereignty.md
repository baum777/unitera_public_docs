...
```mermaid
flowchart LR
    H["Human"] -->|"sets goals and boundaries"| I["Institution"]
    I -->|"bounds use"| C["AI capability"]
    C -->|"returns proposals"| H
```
...
```mermaid
flowchart LR
    R["Reality"] -->|"observe"| K["KNOW"]
    K -->|"context"| T["THINK"]
    T -->|"proposal"| G["Govern"]
    G -->|"authorized action"| A["ACT"]
    A -->|"effect"| P["PROVE"]
    P -->|"verified result"| R
```
...
```mermaid
flowchart LR
    P["Personal Realm"] -->|"explicit contribution"| B["Boundary review"]
    B -->|"admitted context"| C["Company Brain"]
    C -.->|"no silent transfer back"| P
```
...
```mermaid
flowchart LR
    M["Model output"] -->|"proposal"| D["Decision boundary"]
    D -->|"authorized"| E["Bounded effect"]
    E -->|"Receipt"| V["Verification"]
    D -->|"not authorized"| N["No effect"]
```
...
```mermaid
flowchart LR
    S["Retained context"] -->|"restore"| R["Resume"]
    R -->|"re-evaluate"| G["Current boundaries"]
    G -->|"continue"| W["Traceable work"]
    G -->|"not permitted"| X["Stop or ask"]
```
